...
public class EmployeeController {
    ...
    public ResponseEntity getEmployee(@PathVariable String employeeNo) {
        try {
            ...
            if (employeeInfo != null) {
                ...
                return new ResponseEntity<>("Employee not found for employeeNo: " + employeeNo, HttpStatus.NOT_FOUND);
            }

        } catch (Exception e) {
            return new ResponseEntity<>("Failed to get employee information. Error: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity postEmployee(@RequestBody EmployeeInfo employeeInfo) {
        try {
            ...
            return new ResponseEntity<>("Employee information inserted successfully.", HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>("Failed to insert employee information. Error: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    @PostMapping("/modify/{employeeNo}")
    public ResponseEntity updateEmployee(@PathVariable String employeeNo, @RequestBody EmployeeInfo employeeInfo) {
        try {
            employeeInfo.setEmployeeNo(employeeNo);
            employeeService.updateEmployee(employeeInfo);
            return new ResponseEntity<>("Employee information updated successfully.", HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>("Failed to insert employee information. Error: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity deleteEmployee(@PathVariable String employeeNo) {
        try {
            ...
            return new ResponseEntity<>("Employee information deleted successfully.", HttpStatus.OK);
        }catch (Exception e) {
            return new ResponseEntity<>("Failed to insert employee information. Error: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
